...
class Markov(start_list, word_list):
    def __init__(self, start_list, word_list):
        # for sentence in word_list: # Cleaned Corpus
        self.markov_chain = self.create_histograms(start_list, word_list)

    def create_histograms(self, start_list, word_list):
    ...
    #     randomly according to the frequency of words following it."""
        if 'START' in markov:
            ...
            while current_word == 'START':
                current_word = sample_by_frequency(markov['START'])
                return current_word
...
def test_markov_chain():
    ...
    markov_modal = Markov(cleaned_file["start_list"], cleaned_file["word_list"])
    print(markov_modal)
